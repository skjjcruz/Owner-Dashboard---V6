// ══════════════════════════════════════════════════════════════════
// trade-calc-lab.js — TRADE LAB engine (loaded ONLY by trade-lab.html)
//
// A copy of js/trade-calc.js (the b84/b98 production engine) with the
// behavioral model wired in. Production never loads this file; the lab
// never loads production's. Every lab edit is tagged "LAB:" so the P3
// graft is a grep away. Regenerate the shell with
// scripts/build-trade-lab.cjs after index.html changes.
//
// What the lab adds on top of b98:
//  - Points ledger (WrLabPointsLedger): every Why speaks the 243 pace
//    in plain relative language — group rank vs the league, never
//    absolute point gaps and never system vocabulary.
//  - Rival intent (WrLabIntentReads): partners ranked and priced by
//    what their behavior says they're doing; shown softly, no banners.
//    My own advice still follows MY declared GM strategy (declaration
//    wins) — intent reads apply to rivals only.
//  - Six-tier QB rules v2 (with the 40+ age rule, the Rodgers rule and
//    the seller-scarcity bump) replace v1 as the finder's QB gate.
//  - Soft-group targeting: the finder shops the groups my lineup is
//    actually thin in, per the ledger, alongside the legacy needs read.
// ══════════════════════════════════════════════════════════════════
